...
export async function resolveTodoMaxItems(event: H3Event, userId: string): Promise<number | null> {
  ...
  const polarAccessToken = runtimeConfig.polar?.accessToken
  const productId = runtimeConfig.public?.polar?.productId
  ...
  try {
    ...
    if (customerState.activeSubscriptions?.some(subscription => subscription.productId === productId)) {
      return null
    }
  } catch (error) {
