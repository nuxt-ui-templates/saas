...
const todoDescription = computed(() => {
  if (isTodoLoading.value) return 'Each user has one list.'
  if (isTodoProPlan.value) return 'Each user has one list. Pro users have no todo limit.'
  return `Each user has one list. Free users can create up to ${todoMaxItems.value} todos.`
})
...
async function onCreateTodo() {
  if (await createTodo(newTodoTitle.value)) {
    newTodoTitle.value = ''
  }
}
</script>
